document.addEventListener('DOMContentLoaded', async () => {
    ...
    function unsubscribeMatchRealtime() {
        if (matchRealtimeChannel && typeof matchRealtimeChannel.unsubscribe === 'function') {
    ...
    function setupMatchRealtimeSubscription(matchId) {
        ...
        matchRealtimeChannel
            .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'fm_matches', filter: `id=eq.${matchId}` }, (payload) => {
                if (payload?.new) {
                    currentMatch = normalizeMatchFromSupabase(payload.new);
                    renderAllSections();
                }
            })
            .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'fm_matches', filter: `id=eq.${matchId}` }, async () => {
                await FMModal.system({
                    title: 'Partida alterada em tempo real',
                    message: 'A partida foi removida ou alterada. Recarregue a pagina para ver o estado atualizado.',
                    source: 'realtime',
                    priority: 100
                });
    ...
    function getCurrentUserDraw() {
        const draws = normalizeTeamDraws(currentMatch?.teamDraws || currentMatch?.team_draws || {});
    ...
    function getMatchPlayer(username) {
        ...
        return currentMatch.players.find(player => getDrawKey(player?.username) === usernameKey) || null;
    }

    function playerCanDrawAgain(player) {
        return Boolean(player && isScratchCardEnabled() && !getPlayerTeamLabel(player));
    }
    ...
    function createTeamOptions(selectedTeamId = '') {
        ...
        getMatchTeams().forEach(team => {
            ...
            options.push(`<option value="${team.id}" ${selected}>${team.name}</option>`);
        });
        return options.join('');
    }
    ...
    async function loadAndRenderPage() {
        try {
            ...
            currentMatch.teamDraws = normalizeTeamDraws(currentMatch.teamDraws || currentMatch.team_draws || {});
            currentMatch.players = Array.isArray(currentMatch.players) ? currentMatch.players : [];
            currentMatch.votes = currentMatch.votes || { best_player: [], worst_player: [] };
            ...
            if (client) {
                ...
                if (!voteError && voteData) {
                    currentMatch.votes.best_player = voteData
                        ...
                        .map(v => ({ voter: v.voter_username, candidate: v.candidate_username }));
            ...
            if (currentUser && isScratchCardEnabled()) {
                try {
                    ...
                    if (serverStatus && serverStatus.has_draw) {
                        ...
                        if (!existingPlayer) {
                            console.warn('Draw ativo encontrado no servidor, mas jogador nao esta na lista de players. ' +
                            ...
                            try {
                                ...
                                if (client) {
                                    await client.rpc('admin_release_player_draw', {
                                        ...
                                        p_release_reason: 'Draw orfao - jogador nao consta na lista de confirmados.'
                                    });
                                }
                            } catch (e) {
                                console.warn('Nao foi possivel liberar draw orfao:', e);
                            }
                        } else if (!existingPlayer.teamId || existingPlayer.teamId !== serverTeamId) {
                    ...
                    console.warn('Não foi possível verificar status do sorteio no servidor:', statusError);
                    // Continua com os dados locais (modo degradado)
                }
            }
            
            pageTitle.textContent = `Detalhes: ${currentMatch.location}`;
            if (currentMatch.status === 'ENCERRADA' && !currentMatch.results_processed && new Date() > new Date(currentMatch.voting_deadline)) {
    ...
    function renderMatchDetailsInfo() {
        ...
        switch(currentMatch.status) {
            case 'CONFIRMADA':
                statusBadgeStyle = 'background: rgba(16,185,129,0.15); color: #6ee7b7; border: 1px solid rgba(16,185,129,0.3);';
                break;
            case 'AGENDADA':
                statusBadgeStyle = 'background: rgba(59,130,246,0.15); color: #93c5fd; border: 1px solid rgba(59,130,246,0.3);';
                break;
            case 'CANCELADA':
                statusBadgeStyle = 'background: rgba(239,68,68,0.15); color: #fca5a5; border: 1px solid rgba(239,68,68,0.3);';
                break;
            case 'ENCERRADA':
                statusBadgeStyle = 'background: rgba(100,116,139,0.15); color: var(--text-secondary); border: 1px solid rgba(100,116,139,0.3);';
                break;
            default:
                statusBadgeStyle = 'background: rgba(255,255,255,0.1); color: white; border: 1px solid rgba(255,255,255,0.2);';
        }

        const teams = getMatchTeams();
        const teamsSummaryHTML = teams.length > 0 ? `
    ...
    async function renderPlayerList() {
        ...
        if (currentMatch.players.length === 0) {
            container.innerHTML = '<p>Nenhum jogador confirmado ainda.</p>';
            return;
        }
        ...
        if (withdrawnPlayers.length > 0) {
            ...
            withdrawnHeader.className = 'withdrawn-section-header';
            withdrawnHeader.innerHTML = `
                <h4 class="withdrawn-title">⚠️ Jogadores que Desistiram (${withdrawnPlayers.length})</h4>
            `;
            container.appendChild(withdrawnHeader);
    ...
    async function buildPlayerElement(player, isAdmin, client) {
        ...
        if (isAdmin && client) {
            try {
                const { data: phone } = await client.rpc('get_player_phone', { p_username: player.username });
        ...
        let statusBadgeHTML = '';
        if (isWithdrawn) {
            statusBadgeHTML = '<span class="player-status-badge withdrew" title="Jogador desistiu">Desistiu</span>';
        }
        ...
        const detailsButton = `<button class="btn-details" data-username="${player.username}" title="Ver detalhes"><span class="details-icon">👁️</span><span class="details-text">Ver detalhes</span></button>`;

        const whatsappButton = (isAdmin && playerPhone) 
            ...
            : (isAdmin ? `<div class="info-item"><span class="no-phone-info">📵 Sem celular cadastrado</span></div>` : '');
        ...
        const hasDrawLocked = Boolean(player.teamId || player.teamName || currentMatch.teamDraws?.[drawKey]);
        const teamBadge = getMatchTeams().length > 0 && !isWithdrawn
            ...
            : '';
        const teamControl = (isAdmin && getMatchTeams().length > 0 && !isWithdrawn)
            ...
            : (!isWithdrawn && teamLabel ? `<div class="info-item"><span class="info-label">Time:</span><span class="info-value">${teamLabel}</span></div>` : '');
        ...
        const withdrawInfo = isWithdrawn ? `
            ...
            ${player.withdrawReason ? `<div class="info-item withdraw-info">
                <span class="info-label">💬 Motivo:</span>
                <span class="info-value">${player.withdrawReason}</span>
            </div>` : ''}
            ${player.withdrawnAt ? `<div class="info-item withdraw-info">
    ...
    function renderFinancialSummary() {
        ...
        const activePlayers = currentMatch.players.filter(p => p.status !== 'withdrew' && p.status !== 'removed');
    ...
    function isDrawFull() {
        ...
        const players = Array.isArray(currentMatch.players) ? currentMatch.players : [];
        
        let allFull = true;
        for (const team of first3Teams) {
            const count = players.filter(p => p.teamId === team.id || p.teamName === team.name).length;
            if (count < 7) {
                allFull = false;
                break;
            }
    ...
    function createScratchCard() {
        ...
        const selectedTeamName = existingDraw?.teamName || getTeamById(existingDraw?.teamId)?.name || '';
            ...
            setTimeout(() => {
                ...
                if (btn) {
                    btn.addEventListener('click', async () => {
                        btn.disabled = true;
                        btn.innerHTML = '<span class="scratch-spinner" style="display:inline-block; border-color:#fff; border-bottom-color:transparent; width:16px; height:16px; margin-right:8px;"></span> Entrando...';
                        try {
                            const result = await playerDrawTeam(currentMatch.id, fourthTeam.id);
                            if (result && result.assignment) {
                                FMModal.success('Você entrou no ' + result.assignment.teamName + '!');
        ...
        const allSlots = [...teamSlots, ...neutralSlots].sort(() => Math.random() - 0.5);
    ...
    async function handleScratchReveal(event) {
        ...
        if (cardType === 'team') {
            content.innerHTML = '<div class="scratch-loading"><span class="scratch-spinner"></span><span>Sorteando...</span></div>';
        ...
        if (cardType === 'neutral') {
            ...
            if (footerHint) {
                const remainingCovered = container.querySelectorAll('.scratch-card.covered').length;
        ...
        try {
            ...
            if (!drawSuccess) {
                ...
                FMModal.error(error.message || 'Erro ao realizar o sorteio. Tente novamente.');
                return;
            }
        }
        ...
        const teams = getMatchTeams();
        const realTeam = teams.find(t => t.name === assignedTeamName) || { name: assignedTeamName, color: '#10b981' };
        ...
        if (originalCardTeamName && originalCardTeamName !== assignedTeamName) {
            const otherCardWithRealTeam = Array.from(container.querySelectorAll('.scratch-card'))
                .find(c => c !== card && c.dataset.teamName === assignedTeamName && c.dataset.type === 'team');
            ...
            if (otherCardWithRealTeam) {
                ...
                const originalTeamObj = teams.find(t => t.name === originalCardTeamName) || { color: '#10b981' };
                
                otherCardWithRealTeam.dataset.teamName = originalCardTeamName;
                otherCardWithRealTeam.dataset.teamId = originalTeamId || '';
                otherCardWithRealTeam.style.setProperty('--card-color', originalColorStr);
                otherCardWithRealTeam.style.setProperty('--card-rgb', originalRgbStr);
                
                const otherContent = otherCardWithRealTeam.querySelector('.scratch-card-content');
                if (otherContent) {
        ...
        for (const otherCard of allCards) {
            if (otherCard === card || otherCard.classList.contains('revealed')) continue;
            ...
            otherCard.classList.add('revealed', isNeutral ? 'neutral-revealed' : 'not-chosen');
        ...
        shareBtn.innerHTML = '\ud83d\udce9 Enviar lista de jogadores atualizada para administrador';
        shareBtn.style.cssText = 'margin-top: 12px; width: 100%; background: linear-gradient(135deg, #25d366, #128c7e); border: none; color: white; padding: 14px 20px; border-radius: 12px; font-weight: 700; font-size: 0.95rem; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; transition: transform 0.2s, filter 0.2s;';
        shareBtn.addEventListener('mouseenter', () => { shareBtn.style.transform = 'translateY(-2px)'; shareBtn.style.filter = 'brightness(1.1)'; });
        shareBtn.addEventListener('mouseleave', () => { shareBtn.style.transform = 'none'; shareBtn.style.filter = 'none'; });
    ...
    function renderActionButtons() {
        ...
        if (isVisitor) {
            ...
            visitorMsg.style.cssText = 'border-color: rgba(245,158,11,0.3); background: rgba(245,158,11,0.06);';
        ...
        if (isPlayerInMatch) {
            ...
            if (playerHasTeam && isScratchCardEnabled()) {
                ...
                shareButton.addEventListener('mouseenter', () => { shareButton.style.transform = 'translateY(-2px)'; shareButton.style.filter = 'brightness(1.1)'; });
            ...
            actionButtonsContainer.appendChild(createActionButton('Quero Participar', 'btn-primary', handleJoinMatch));
        }
    }
    ...
    function renderAdminControls() {
        ...
        const container = adminControlsSection.querySelector('.admin-buttons-container');
        container.innerHTML = '';
        
        const playerManager = document.createElement('div');
        playerManager.className = 'admin-player-management';
        const teamSelectHTML = getMatchTeams().length > 0 ? `
        ...
        playerManager.querySelector('#add-player-btn').addEventListener('click', handleAdminAddPlayer);
        ...
        teamManager.querySelector('#edit-teams-btn').addEventListener('click', () => {
            sessionStorage.setItem('editMatchId', currentMatch.id);
            window.location.href = 'schedule.html';
        });
        
        // Controles de status da partida
        if (currentMatch.status === 'ENCERRADA' && !currentMatch.results_processed) {
            container.appendChild(createAdminButton('Apurar Votos e Finalizar', 'btn-confirm', () => processVotingResults(currentMatch)));
        }
        if (currentMatch.status !== 'CONFIRMADA' && currentMatch.status !== 'ENCERRADA') {
            container.appendChild(createAdminButton('Confirmar Partida', 'btn-confirm', () => updateMatchStatus('CONFIRMADA')));
        }
        if (currentMatch.status !== 'CANCELADA' && currentMatch.status !== 'ENCERRADA') {
            container.appendChild(createAdminButton('Cancelar Partida', 'btn-cancel', () => updateMatchStatus('CANCELADA')));
        }
        if (currentMatch.status !== 'ENCERRADA') {
            container.appendChild(createAdminButton('Encerrar Partida', 'btn-secondary', () => updateMatchStatus('ENCERRADA')));
        }
        if (['CONFIRMADA', 'CANCELADA', 'ENCERRADA'].includes(currentMatch.status)) {
            container.appendChild(createAdminButton('Reagendar', 'btn-reschedule', () => updateMatchStatus('AGENDADA')));
        }
    }

    function renderVotingSection() {
        const bestPlayerVotesContainer = document.getElementById('best-player-votes');
        const worstPlayerVotesContainer = document.getElementById('worst-player-votes');
        ...
        if (currentMatch.results_processed) {
            voteMessage.innerHTML = "<span style='color:var(--accent-green);font-weight:600'>Votação encerrada e resultados apurados.</span>";
            
            const findWinner = (votes) => {
                if (!votes || votes.length === 0) return null;
                const voteCounts = votes.reduce((acc, vote) => { acc[vote.candidate] = (acc[vote.candidate] || 0) + 1; return acc; }, {});
                const sortedVotes = Object.entries(voteCounts).sort((a, b) => b[1] - a[1]);
                return sortedVotes.length > 0 ? { name: sortedVotes[0][0], count: sortedVotes[0][1] } : null;
            };
        ...
        if (isVisitor) {
            ...
            bestPlayerVotesContainer.innerHTML = '<p class="no-votes" style="color: var(--text-muted);">Fa\u00e7a login como jogador para votar.</p>';
            worstPlayerVotesContainer.innerHTML = '<p class="no-votes" style="color: var(--text-muted);">Fa\u00e7a login como jogador para votar.</p>';
            return;
        }
        ...
        countdownInterval = setInterval(() => {
            ...
            if (distance < 0) {
                ...
                voteMessage.innerHTML = 'Tempo esgotado! Recarregue a página para apurar os resultados.';
                return;
            }
        ...
        const userHasVotedBest = currentMatch.votes.best_player.some(v => v.voter === currentUser.username);
        const userHasVotedWorst = currentMatch.votes.worst_player.some(v => v.voter === currentUser.username);

        // Progresso de votação
        const totalVoters = currentMatch.players.filter(p => p.status !== 'withdrew' && p.status !== 'removed').length;
        const uniqueBestVoters = new Set(currentMatch.votes.best_player.map(v => v.voter)).size;
        const uniqueWorstVoters = new Set(currentMatch.votes.worst_player.map(v => v.voter)).size;
        ...
        bestPlayerVotesContainer.innerHTML = statusBestHtml + (userHasVotedBest ? '<div class="vote-confirmed-badge">✅ Seu voto foi registrado</div>' : '');
        worstPlayerVotesContainer.innerHTML = statusWorstHtml + (userHasVotedWorst ? '<div class="vote-confirmed-badge">✅ Seu voto foi registrado</div>' : '');

        // Lista de candidatos (excluindo o próprio jogador)
        const activePlayers = currentMatch.players.filter(p => p.status !== 'withdrew' && p.status !== 'removed');
        activePlayers.forEach(player => {
            const votesForPlayerBest = currentMatch.votes.best_player.filter(v => v.candidate === player.username).length;
            bestPlayerVotesContainer.appendChild(createCandidateElement(player, votesForPlayerBest, 'best_player', userHasVotedBest));
            const votesForPlayerWorst = currentMatch.votes.worst_player.filter(v => v.candidate === player.username).length;
            worstPlayerVotesContainer.appendChild(createCandidateElement(player, votesForPlayerWorst, 'worst_player', userHasVotedWorst));
        });
    }
    ...
    function renderFinancialsButton() {
        ...
        const financialBtn = createActionButton('📊 Ver Resumo Financeiro', 'btn-primary', () => {
    ...
    async function handleJoinMatch() {
        ...
        if (!currentUser.username) {
            await FMModal.warning("Seu cadastro parece estar incompleto ou a sessao expirou. Por favor, faca login novamente.");
        ...
        if (isScratchCardEnabled()) {
            if (typeof isDrawFull === 'function' && isDrawFull()) {
                FMModal.drawConfirm('As vagas para o sorteio acabaram! Role a tela para baixo e entre diretamente no 4º time.');
            } else {
                FMModal.drawConfirm('Esta partida possui sorteio de times. Use o sorteio para participar.');
            }
            renderActionButtons();
            return;
        }
        
        console.log('🎯 Iniciando processo de participação na partida diretamente...');
        
        // Verificar se o jogador já está na partida
        const isPlayerInMatch = currentMatch.players.some(p => getDrawKey(p.username) === getDrawKey(currentUser.username));
        if (isPlayerInMatch) {
        ...
        try {
            ...
            await playerUpdateMatchData(currentMatch.id, currentMatch.players, currentMatch.votes);
    ...
    async function handleAdminAddPlayer() {
        ...
        const playerExists = currentMatch.players.some(p => p.username.toLowerCase() === playerName.toLowerCase());
        if (playerExists) {
            FMModal.warning('Este jogador ja esta na partida.');
            return;
        }

        const selectedTeam = playerTeamSelect?.value ? getTeamById(playerTeamSelect.value) : null;
        ...
        try {
            ...
            currentMatch.players = currentMatch.players.filter(p => p.username !== playerName);
            console.error('Erro ao adicionar jogador:', error);
            FMModal.error(error?.message || 'Erro ao adicionar jogador. Tente novamente.');
        }
    }
    ...
    async function handleRemovePlayer(event) {
        ...
        const previousDraws = currentMatch.teamDraws ? { ...currentMatch.teamDraws } : {};
        const previousSnakeDraws = currentMatch.team_draws ? { ...currentMatch.team_draws } : null;
        const previousVotes = {
        ...
        currentMatch.votes = currentMatch.votes || { best_player: [], worst_player: [] };
        currentMatch.votes.best_player = (currentMatch.votes.best_player || []).filter(vote => {
            return normalizeTeamKey(vote?.voter) !== usernameKey
                && normalizeTeamKey(vote?.candidate) !== usernameKey;
        });
        currentMatch.votes.worst_player = (currentMatch.votes.worst_player || []).filter(vote => {
            return normalizeTeamKey(vote?.voter) !== usernameKey
                && normalizeTeamKey(vote?.candidate) !== usernameKey;
        });
        ...
        try {
            ...
            FMModal.error(error?.message || 'Erro ao remover jogador. Tente novamente.');
        }
    }
    ...
    async function handleReleasePlayerDraw(event) {
        ...
        try {
            ...
            FMModal.error(error?.message || 'Erro ao liberar novo sorteio. Tente novamente.');
        }
    }
    ...
    async function handlePlayerTeamChange(event) {
        ...
        try {
            ...
            FMModal.error(error?.message || 'Erro ao mover jogador. Tente novamente.');
        }
    }
    ...
    function handleWhatsAppPlayer(event) {
        ...
        const message = `*⚽ Futebol Milhão - Mensagem do Admin*\n\n` +
            `Olá *${username}*! 👋\n\n` +
    ...
    async function confirmLeaveMatch(reason) {
        ...
        try {
            ...
            const message = `*⚠️ DESISTÊNCIA DE JOGADOR*\n\n` +
            ...
            loadingModal.close();
            FMModal.success("Sua desistência foi registrada com sucesso.\nVocê permanece na lista, mas sem time.\nO administrador foi notificado.");
            loadAndRenderPage();

        } catch (error) {
            loadingModal.close();
            console.error('Erro ao registrar desistência:', error);
            FMModal.error(error?.message || "Erro ao registrar desistência. Tente novamente.");
        }
    }
        ...
        try {
            ...
            if (newStatus === 'ENCERRADA') {
                FMModal.voteOpen({
                    ...
                    message: 'A partida foi encerrada. A votacao pos-jogo esta aberta por 24 horas.'
                });
            } else {
            ...
            if (currentUser.role === 'admin') {
                ...
                if (adminPhoneNumber) {
                    const [year, month, day] = currentMatch.date.split('-');
                    const formattedDate = `${day}/${month}/${year}`;
                    const message = `📢 *ATENÇÃO! Status da Partida Alterado!* 📢\n\n` +
                    ...
                    FMModal.warning("Numero do administrador nao configurado para notificacoes WhatsApp.");
                }
            }
    ...
    async function handleVote(event) {
        ...
        try {
            ...
            if (error) {
                if (error.code === '23505' || (error.message && error.message.includes('unique_violation'))) {
            ...
            FMModal.error(error.message || "Nao foi possivel registrar seu voto. Tente novamente.");
    ...
    function createCandidateElement(player, voteCount, category, userHasVoted) {
        ...
        const hasVotedForThis = currentMatch.votes[category].some(v => v.voter === currentUser.username && v.candidate === player.username);
        const isSelf = normalizeTeamKey(player.username) === normalizeTeamKey(currentUser.username);
        const isVisitor = currentUser.role === 'visitor';
        ...
        const barColor = category === 'best_player' ? 'var(--accent-green, #10b981)' : 'var(--accent-red, #ef4444)';
    ...
    async function handleShareTeamList() {
        ...
        const activePlayers = currentMatch.players.filter(p => 
            p.status !== 'withdrew' && p.status !== 'removed'
        );
        ...
        activePlayers.forEach(player => {
            ...
            if (finalTeamId && playersByTeam.has(finalTeamId) && !assignedPlayers.has(drawKey)) {
